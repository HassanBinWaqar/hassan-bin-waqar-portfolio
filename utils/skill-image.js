...
import softwareArchitecture from '../app/assets/svg/skills/software-architecture.svg';
...
export const skillsImage = (skill) => {
  ...
  // Normalize skill string: remove parenthetical notes like "(DSA)", trim and lower-case
  let skillID = String(skill)
    .replace(/\s*\([^)]*\)\s*/g, '') // remove parentheses and their contents
    .trim()
    .toLowerCase();

  // Map common synonyms/variants to existing icons
  if (skillID === 'sql' || skillID === 'sql dbms') {
    // use MySQL icon as a generic SQL icon (mysql.svg exists)
    skillID = 'mysql';
  }

  // Map software design / architecture to a design icon (figma)
  if (skillID.includes('design') || skillID.includes('architecture')) {
    // prefer a dedicated software-architecture icon when available
    skillID = 'software-architecture';
  }

  if (skillID === 'full stack development' || skillID === 'fullstack' || skillID === 'full stack' || skillID === 'full-stack') {
    // represent full-stack with a popular frontend icon (React) as a sensible default
    skillID = 'react';
  }
